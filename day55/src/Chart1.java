...
public class Chart1 {
    ...
    public Chart1(String title, String title1,String title2,String title3) throws SQLException {
        XYDataset xydataset = createDataset(title1,title2,title3);
        JFreeChart jfreechart = ChartFactory.createTimeSeriesChart(title, "年度", "增长率(%)",xydataset, true, true, true);
        XYPlot xyplot = (XYPlot) jfreechart.getPlot();
        DateAxis dateaxis = (DateAxis) xyplot.getDomainAxis();
        dateaxis.setDateFormatOverride(new SimpleDateFormat("yyyy"));
    ...
    private static TimeSeries getTimeSeries(String title) throws SQLException {
        ...
        try(Connection connection = DUtil.getConnection()) {
            String sql = "select a2014,a2015,a2016,a2017,a2018,a2019 from a201001 where item = ?";
            try(PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                try(ResultSet r = ps.executeQuery()) {
                    if(r.next()) {
                        ...
                        timeseries.add(new Year(2014),func(tAssets_2014,tAssets_2015)*100D);
                        timeseries.add(new Year(2015),func(tAssets_2015,tAssets_2016)*100D);
                        timeseries.add(new Year(2016),func(tAssets_2016,tAssets_2017)*100D);
                        timeseries.add(new Year(2017),func(tAssets_2017,tAssets_2018)*100D);
                        timeseries.add(new Year(2018),func(tAssets_2018,tAssets_2019)*100D);
                    }
                }
            }
        }
    ...
    private static XYDataset createDataset(String title1,String title2,String title3) throws SQLException {  //这个数据集有点多，但都不难理解
        TimeSeriesCollection timeseriescollection = new TimeSeriesCollection();
        timeseriescollection.addSeries(getTimeSeries(title1));
        timeseriescollection.addSeries(getTimeSeries(title2));
        timeseriescollection.addSeries(getTimeSeries(title3));
